...
public class ExtentReportManager {
	...
	public ExtentReportManager() {
		...
		ExtentSparkReporter sparkReporter = new ExtentSparkReporter(reportFolderPath + "\\SnapDealReport_"+  folderName+".html");

		sparkReporter.config().setDocumentTitle("Automation Extent Report");
		sparkReporter.config().setReportName("Snapdeal Test Report");
		sparkReporter.config().setTheme(Theme.STANDARD);
		sparkReporter.config().setTimeStampFormat("EEEE, MMMM dd, yyyy, hh:mm:ss a '('zzz')'");
		extentreport = new ExtentReports();
		extentreport.attachReporter(sparkReporter);
		// testLogger = extentreport.createTest("Zigly search products");
	}
